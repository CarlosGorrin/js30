...
function getVideo() {
	...
	.catch(err => {
		console.error(`OH NO!!! Hey man! Connect your camera`, err);
	});
}
...
function takePhoto() {
	...
	const data = canvas.toDatURL('image/jpeg');
	...
	function redEffect(pixels) {
		for(let i = 0; i < pixels.data.lenght; i+=4) {
			...
			pixels[i + 0] = pixels.data[i + 1] - 50; //green
			pixels[i + 0] = pixels.data[i + 2] * 0.5; //blue
		}
		return pixels;
	}
